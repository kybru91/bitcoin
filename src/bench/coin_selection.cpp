...
static void CoinSelection(benchmark::Bench& bench)
{
    ...
    for (const auto& wtx : wtxs) {
        coins.emplace_back(wtx.get(), 0 /* iIn */, 6 * 24 /* nDepthIn */, true /* spendable */, true /* solvable */, true /* safe */, wtx->IsFromMe(ISMINE_ALL), wtx->GetSpendSize(0));
    }
...
static void add_coin(const CAmount& nValue, int nInput, std::vector<OutputGroup>& set)
{
    ...
    set.emplace_back();
    set.back().Insert(COutput(wtx.get(), nInput, 0, true, true, true, true, wtx->GetSpendSize(nInput)), 0, 0, false);
