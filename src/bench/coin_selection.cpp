...
static void addCoin(const CAmount& nValue, CWallet& wallet, std::vector<std::unique_ptr<CWalletTx>>& wtxs)
{
...
static void CoinSelection(benchmark::State& state)
{
    ...
    for (const auto& wtx : wtxs) {
        COutput output(wtx->GetHash(), 0 /* iIn */, wtx->GetTx()->vout[0], 6 * 24 /* nDepthIn */, true /* spendable */, true /* solvable */, true /* safe */, &wallet);
...
static void add_coin(const CAmount& nValue, int nInput, std::vector<OutputGroup>& set)
{
    ...
    set.emplace_back(COutput(wtx->GetHash(), nInput, wtx->GetTx()->vout[nInput], 0, true, true, true, &testWallet).GetInputCoin(), 0, true, 0, 0);
