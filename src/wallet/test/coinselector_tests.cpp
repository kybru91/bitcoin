...
static void add_coin(CWallet& wallet, const CAmount& nValue, int nAge = 6*24, bool fIsFromMe = false, int nInput=0, bool spendable = false)
{
    ...
    COutput output(wtx->tx->vout[nInput], COutPoint(wtx->GetHash(), nInput), nAge, true /* spendable */, true /* solvable */, true /* safe */, fIsFromMe, wtx->GetSpendSize(nInput, true), wtx->GetTxTime());
