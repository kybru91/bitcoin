...
private:
    ...
    bool AddKeyPubKeyWithDB(WalletBatch &batch,const CKey& key, const CPubKey &pubkey, CKeyMetadata* meta = nullptr) EXCLUSIVE_LOCKS_REQUIRED(cs_wallet);
    ...
    std::map<CKeyID, CKeyMetadata> m_keymeta_to_upgrade GUARDED_BY(cs_wallet);
...
public:
    ...
    bool AddKeyPubKey(const CKey& key, const CPubKey &pubkey, CKeyMetadata* meta) EXCLUSIVE_LOCKS_REQUIRED(cs_wallet);
    ...
    bool ImportPubKeys(const std::vector<CKeyID>& ordered_pubkeys, const std::map<CKeyID, CPubKey>& pubkey_map, const std::map<CPubKey, KeyOriginInfo>& key_origins, const bool add_keypool, const bool internal, const int64_t timestamp) EXCLUSIVE_LOCKS_REQUIRED(cs_wallet);
    ...
    bool GetKeyOrigin(const CPubKey& pubkey, KeyOriginInfo& info) const override;
};
