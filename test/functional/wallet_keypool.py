...
class KeyPoolTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # Creating a transaction with the exact fee subtracted from the amount should work
        ...
        res = w2.walletcreatefundedpsbt(inputs=[], outputs=[{destination: 0.00000294}], options={"feeRate": 0.0008823})
        assert_equal("psbt" in res, True)
        assert_equal(res["fee"], Decimal("0.00009706"))

        # creating a 10,000 sat transaction with a manual change address should be possible
        res = w2.walletcreatefundedpsbt(inputs=[], outputs=[{destination: 0.00008900}], options={"feeRate": 0.00010, "changeAddress": addr.pop()})
        assert_equal("psbt" in res, True)
